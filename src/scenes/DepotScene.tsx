...
export const DepotScene: React.FC<{ onDispatch: (vehicleId: string) => void }> = ({ onDispatch }) => {
    ...
    return (
        <div aria-label="Depot Planning" style={{
            ...
            {/* TOP/LEFT SECTION: Intro + Actions + PARCELS (Horizontal Scroller on Mobile) */}
            <div style={{
                minHeight: 0,
                display: 'flex', // ADDED
                flexDirection: 'column', // ADDED
                flex: isMobile ? 'none' : '0 0 420px', // CHANGED (fixed width on desktop)
            }}>
                ...
                {/* --- THIS IS THE KEY MOBILE FIX: HORIZONTAL SCROLLER --- */}
                <div style={{
                    display: 'flex', // CHANGED from 'grid'
                    flexDirection: isMobile ? 'row' : 'column', // CHANGED
                    gap: 10,
                    overflowX: isMobile ? 'auto' : 'hidden', // CHANGED
                    overflowY: isMobile ? 'hidden' : 'auto', // CHANGED
                    paddingBottom: isMobile ? '10px' : '0', // ADDED spacing for scrollbar
                    paddingRight: isMobile ? '0' : '4px',
                    flexShrink: 0, // ADDED (prevents shrinking on mobile)
                }}>
                    {unassignedParcels.map((p) => (
                        // ACCESSIBILITY FIX: Changed div to button
                        <button
                            key={p.id}
                            ...
                            style={{
                                ...
                                position: 'relative',
                                zIndex: 1,
                                flex: isMobile ? '0 0 200px' : '1', // ADDED fixed width for mobile cards
                                textAlign: 'left', // ADDED (button default is center)
                                color: '#fff', // ADDED (button default)
                                fontFamily: 'inherit', // ADDED (button default)
                            }}>
            ...
            {/* BOTTOM/RIGHT SECTION: Vehicles (Main Vertical Scroller on Mobile) */}
            <div style={{
                flex: 1, // ADDED (takes remaining space)
                overflowY: 'auto', // ADDED (this section scrolls)
                minHeight: 0, // ADDED (critical for flex scrolling)
                paddingRight: '4px', // ADDED (for scrollbar)
            }}>
                ...
                <div style={{
                    ...
                    {vehicles.map((v) => {
                        ...
                        // Check if this vehicle is a valid tap target
                        const isTapTarget = selectedParcelId && !isDispatched;

                        return (
                            // USABILITY FIX: This card is now a div, not a button, to prevent nesting.
                            <div 
                                key={v.id}
                                onClick={isTapTarget ? () => handleVehicleTap(v.id) : undefined} // Only clickable if a parcel is selected
                                style={{
                                    background: isDispatched ? '#374151' : isTapTarget ? '#1f2937' : '#111827',
                                    padding: isMobile ? 14 : 16,
                                    borderRadius: 12,
                                    cursor: isTapTarget ? 'pointer' : 'default', // Only show pointer if clickable
                                    border: isTapTarget ? '2px solid #3b82f6' : '2px solid transparent',
