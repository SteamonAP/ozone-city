		...
		<div aria-label="Depot Planning" style={{
			...
			<div style={{
				...
				<div style={{ flex: isMobile ? '0 0 auto' : undefined }}>
					<h2 style={{ marginTop: 0, fontSize: isMobile ? '18px' : '24px' }}>🌍 Climate Action Depot</h2>
					<p style={{ marginBottom: 12, opacity: 0.9, fontSize: isMobile ? '13px' : '16px', lineHeight: '1.4' }}>Choose efficient vehicles to minimize emissions and deliver essential supplies to the community.</p>
					...
					<div style={{ background: '#0b1020', padding: isMobile ? 10 : 12, borderRadius: 10, marginBottom: 12 }}>
						...
						<p style={{ margin: 0, fontSize: isMobile ? '11px' : '12px', opacity: 0.8, lineHeight: '1.4' }}>Optimize delivery routes to reduce carbon emissions while serving schools, hospitals, and homes efficiently.</p>
					</div>
					...
					<div style={{
						...
						{unassignedParcels.map((p) => (
							<div key={p.id}
								...
								<div style={{ flex: 1, minWidth: 0 }}>
									<div style={{ fontSize: isMobile ? '14px' : '14px' }}><strong>Parcel {p.id}</strong> — {p.destinationType}</div>
									<div style={{ fontSize: isMobile ? '11px' : '11px', opacity: 0.8, marginTop: 2, lineHeight: '1.3' }}>{getSDGMessage(p.destinationType, p.urgency)}</div>
									<div style={{ fontSize: isMobile ? '10px' : '10px', opacity: 0.6, marginTop: 2 }}>Priority: {p.urgency} • Size: {p.sizeUnits}</div>
									{selectedParcelId === p.id && (
										<div style={{ fontSize: isMobile ? '11px' : '10px', color: '#3b82f6', marginTop: 4, fontWeight: 600 }}>
						...
						{unassignedParcels.length === 0 && <div style={{ opacity: 0.8, textAlign: 'center', padding: isMobile ? 16 : 20, fontSize: isMobile ? '14px' : '16px' }}>✅ All parcels assigned!</div>}
					</div>
				</div>
				...
				<div style={{
					...
					<h3 style={{ marginTop: isMobile ? 16 : 0, marginBottom: 12, fontSize: isMobile ? '16px' : '20px' }}>🚛 Dispatch Vehicles</h3>
					<div style={{
						display: 'grid',
						gap: 12,
							...
							return (
								<div key={v.id}
									...
									<div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 10 }}>
										...
										<div style={{ flex: 1 }}>
											<div style={{ fontSize: isMobile ? '15px' : '16px' }}><strong>{v.id.toUpperCase()}</strong></div>
											<div style={{ opacity: 0.8, fontSize: isMobile ? '12px' : '13px' }}>Capacity {used}/{v.capacity}</div>
